...
11. **workspacePort iframe path rules**: (a) `workspacePort.path` is an HTTP route on the server, NOT a workspace file path — verify with `curl` before using. If the server serves at `/`, pass only `{"port": 8766}`. (b) Server API calls may use relative paths (`fetch('offer')`) or root-relative paths (`fetch('/offer')`); both are forwarded through the dedicated artifact origin. (c) `/artifact-bridge-v1.js` is reserved and served by SilverRetort.
