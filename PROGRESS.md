...
- [x] #7 apps/desktop：`main.cjs` 已接入 hermes 托管
  ...
  - Docker 托管远端模式：`DATA_DIR/settings.json` 额外含 `hermesDockerImage` 时，desktop 先用当前 `DOCKER_HOST` 执行 `docker run` 拉起容器，再继续按 `hermesUrl` 作为远端服务接入；容器固定带 `HERMES_RELAY_ENABLED=1`，`hermesApiKey` 留空时由 desktop 按次随机生成
